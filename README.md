...
运行 lua、busybox 都出现了缺页。但并非因为系统调用未实现。

排查了一天，怀疑是因为加载出了问题。但之前从未了解过工业界系统加载应用时的规范，所以浪费了不少时间。

## 2023-1-7

参考一些文章初步实现了加载器，可以跑了，等着实现系统调用。

## 2023-1-8

实现了系统调用之后也不能跑，看反汇编，应该是在进入 `main` 函数之前的初始化就出错了。我认为可能是动态加载的问题，所以在看 risc-v 的 elf 规范，接下来要进一步完善加载器。
